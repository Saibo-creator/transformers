...
class PromptLookupCandidateGenerator(CandidateGenerator):
    ...
    def get_candidates(self, input_ids: torch.LongTensor) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        for ngram_size in range(min(self.max_matching_ngram_size, input_length - 1), 0, -1):
            ...
            for idx in match_indices:
                ...
                if start_idx < end_idx:
                    ...
                    match_found = True
                    print(f"match_size: {ngram_size}, start_idx: {start_idx}, end_idx: {end_idx}, chosen_ids: {chosen_ids}")
                    break
            if match_found:
                break
        ...
        if match_found:
            print(f"Match found, length: {len(chosen_ids)}, content: {chosen_ids}")
        else:
...
class ExternalDocLookupCandidateGenerator(CandidateGenerator):
    """
    `CandidateGenerator` class to be used for prompt lookup generation. This class generates candidates by looking up
    likely continuations in the provided prompt (input_ids) itself.
    Read the following blog post for more information: https://github.com/apoorvumang/prompt-lookup-decoding
    ...
    def __init__(
        ...
        # Check if the string is a path to an existing file
        if os.path.isfile(doc):
            # If it's a file, read the content
            with open(doc, 'r') as file:
                content = file.read()
        else:
            # Otherwise, assume `doc` is the content
            content = doc

        self.doc_ids: torch.LongTensor = assistant_tokenizer.encode(content, return_tensors="pt")
    ...
    def get_candidates(self, input_ids: torch.LongTensor) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        input_length = input_ids.size(1)
        doc_length = self.doc_ids.size(1)

        chosen_ids = None
        match_found = False
        for ngram_size in range(min(self.max_matching_ngram_size, doc_length - 1), 0, -1):
            # Create sliding windows of size ngram_size
            key_ngram_tensor = self.doc_ids.unfold(dimension=1, size=ngram_size, step=1)

            print(f"key_ngram_tensor: {key_ngram_tensor}")

            # Convert ngram to a tensor for comparison
            query_ngram_tensor = input_ids[0, -ngram_size:]

            print(f"query_ngram_tensor: {query_ngram_tensor}")


            # Find where the windows match the ngram
            matches = (key_ngram_tensor == query_ngram_tensor).all(dim=2)
            ...
            for idx in match_indices:
                ...
                if start_idx < end_idx:
                    chosen_ids = self.doc_ids[0, start_idx:end_idx]
                    match_found = True
                    print(f"match_size: {ngram_size}, start_idx: {start_idx}, end_idx: {end_idx}, chosen_ids: {chosen_ids}")
                    break
            if match_found:
